...
工程验证以 RFC 5545 附录 A 和 graham/rrule 测试语料为参照，核心结果使用断言测试；HTTP/CalDAV 另有真实 socket + curl 集成序列。CI 覆盖所有受支持目标、严格警告、格式、接口文件和 native 服务端构建。Thunderbird 155.0.1 已真实完成发现、读取、创建、修改和删除。项目提供命令行 demo、服务器启动说明、协议边界与客户端互操作矩阵。
...
首版明确不包含完整 IANA tzdb、CalDAV 调度/ACL/TLS、CalDAV 客户端或云平台同步适配；命名时区采用内嵌 VTIMEZONE 优先、常见固定偏移兜底。0.1.0 已发布到 mooncakes.io，形成边界清晰、可测试、可由真实日历客户端直连的 MoonBit 日历基础设施。
